...
error, as is a pin on a Part or lane that does not exist.
...
every time; under `continuous` it lands once. Two pins that resolve to one dice are an
error that names both shapes. `inspect` marks a forced draw with `pinned = true` on the
decision it reached. Pins live with the composition, not a phrase: every phrase and
section inherits them, and the seed is not part of the address, so changing the seed
rerolls everything except the pinned draws — which is what pins are for.
